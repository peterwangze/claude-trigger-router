...
- [x] **Step 1: 在 `src/trigger/types.ts` 末尾追加类型定义**
...
- [x] **Step 3: Commit**: 新增 SmartRouter 默认配置常量

**Files:**
- Modify: `src/constants.ts`

- [x] **Step 1: 在 `src/constants.ts` 中新增默认配置**
...
- [x] **Step 3: 创建 `src/trigger/smart-router.ts` 实现**
...
- [x] **Step 1: 修改 `selector.ts`，添加 SmartRouter 集成**
...
- [x] **Step 1: 修改 `src/trigger/index.ts`**
...
- [x] **Step 1: 确保 smart-router 模块被导出**
...
- [x] **Step 1: 导入 `DEFAULT_SMART_ROUTER_CONFIG` 并接线到 `initConfig()` 的 deepMerge**
...
- [x] **Step 2: 在 `validateConfig()` 函数中添加 SmartRouter 验证**
...
- [x] **Step 1: 在 `TriggerRouter` 配置后追加 SmartRouter 示例**
...
- [x] **Step 1: 在路由优先级表中插入 SmartRouter 行**
...
- [x] **Step 2: 在配置参数参考部分新增 SmartRouter 章节**
...
- [x] `npx vitest run` 全部通过（新增 smart-router 测试 + 无回归）
- [x] `npm run build` 无编译错误
- [x] `config/trigger.example.yaml` 包含 SmartRouter 示例并可通过 YAML 解析
- [x] `README.md` 路由优先级表已更新，SmartRouter 文档完整
- [x] 所有修改均有对应 git commit
