...
- P1-5 `智能路由收益与切换体感闭环`（Chunk 5）：已新增 `qualityEvidence` 真实 trace 样本摘要，`/api/governance/metrics` 会从现有治理 trace 中沉淀 cascade failure、model pool fallback、shadow verification、context window guard、slow request 与 alignment continuity，并统计 failure / improvement / speed risk 样本数；CSV 导出和 `/ui` 维护者工作台同步展示样本类型、severity、requestId、route reason、evidence 和 action。当前仍不宣称已完成同任务多模型 A/B 质量评测。
- P1-5 `智能路由收益与切换体感闭环`（Chunk 6）：已新增 `taskComparison` 任务集对比摘要，`/api/governance/metrics` 会基于已有 governance trace 按任务意图对比不同最终模型的失败率、平均延迟、alignment 使用率与 cascade 率，并选出 baseline / best / fastest model；CSV 导出和 `/ui` 维护者工作台同步展示同任务后验对比。当前仍不宣称已完成离线固定任务集执行器或严格质量评分 rubric。
...
- P1-5 `智能路由收益与切换体感闭环`（Chunk 10）：已新增严格质量维度 rubric，固定任务会从语义覆盖、完整性、交付格式和安全/卫生维度输出 `dimensionScores` 与维度均分；自定义任务也可显式定义 `qualityDimensions`，使同任务多模型 A/B 不只看总分，还能解释模型差在覆盖、格式、完整性还是禁忌内容。当前仍不宣称已完成 LLM 裁判/人工校准入口或 UI/治理面自动化 benchmark 看板。
...
- P1-6 `服务端 API key 与鉴权控制面`（Chunk 3c-c）：已收紧运行时全局权限矩阵，`client` key 只覆盖模型调用，`read-only` key 覆盖健康、服务状态、compiled models、transformers 和 governance 观测 GET 接口，配置读取/保存、`/ui` 和 auth 管理需要 `admin`；README 与中间件回归测试同步 scope 语义。重启和治理维护写操作已在后续 Chunk 3c-e 改为可由 `operator` 承接。
...
- P1-6 `服务端 API key 与鉴权控制面`（Chunk 3c-d）：已在 `/ui` 维护者工作台新增 Auth scope guide，明确 admin、operator、client、read-only 与 `client + read-only` 的适用场景；`ctr doctor` 同步输出 scope 指引与远程 token 发放建议，降低服务使用者误用 admin key 或用纯 client key 做状态探测的概率。
...
- P1-6 Chunk 3c-e 复审补强：已修正 managed key scope 校验会 trim 但运行时授权直接读取原始字符串的问题；现在运行时会在 `scopeAllows()`、`verifyApiKey()` 和脱敏列表输出前归一化已存储 scope，手写配置中的 `operator ` / `read-only ` 不会出现“校验通过但授权失败”，未知 scope 也不会被默认提升为 client。
...
- 部署形态与远程接入收敛（Chunk 4）：已新增远端注册只读摘要同步。remote client 调用本地 `/api/remote-status` 时，会同时探测远端 `/api/service-info` 与 `/api/registration`，使用同一个 `Runtime.remote_service.auth_token`，并返回远端注册模型数、upstream 服务数和脱敏列表；`/ui` 首屏新增 Remote registration 状态。当前仍不把远端注册写回本地配置，也不做服务发现、节点/集群编排或托管控制面。
...
- 本轮闭环判定：UI DOM smoke 已满足 v1.5.0 首轮“基础交互可看护”标准；`workbench-document.ts` 已先拆出 HTML 文档骨架和脚本抽取，并新增脚本语法 smoke。真实浏览器截图、键鼠流程和更深入的 `workbench.ts` CSS/JS/渲染片段拆分后续仍按 P2 继续推进，不把完整 UI 工作台收敛标记为 closed。
