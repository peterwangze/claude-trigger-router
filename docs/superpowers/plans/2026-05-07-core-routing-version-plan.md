...
6. `[closed 2026-05-19]` UI 基础交互可看护首轮：已从 HTML 字符串 smoke 推进到 jsdom DOM smoke，覆盖载入配置、compiled models 预览、保存失败提示、服务状态和维护者 Health action；后续继续拆分 `src/ui/workbench.ts` 的 CSS/JS/渲染片段，避免 UI 继续停留在单个长字符串文件里。
