...
| 8 | 治理观测增强 / 运营化 | closed | v1.18.0 已完成 outcome scorecard、Operations risk、Guardrail summary 和维护者 decision rail | 作为回归底线维护；若 `/ui` 运营信号、scorecard、guardrail summary、pool/key 联动或 browser smoke 再次退化，按 P1/P2 重新前置 |
| 9 | 部署形态与远程接入收敛 | closed | v1.19.0 已完成 remote discovery、remote availability、service-scope 边界、doctor/setup 远程口径和 server/client guidance alignment | 作为回归底线维护；若 remote discovery / availability、远端 registration 摘要、thin proxy 转发或 server/client 文档口径退化，按 P1/P2 重新前置 |
