...
- `智能路由收益与切换体感治理`
  ...
  - 仍需继续补 LLM 裁判/人工校准入口，以及 UI/治理面 benchmark 摘要，让同任务多模型 A/B 从“端到端可重复跑分”推进到“可运营的收益评估”。
- `服务端 API key 与鉴权控制面`
...
|---|---|---|---|---|
| 1 | 智能路由收益与切换体感治理 | P1-主路径易用性 | 在 `ctr eval --run` 与严格质量维度 rubric 之后，继续补 LLM 裁判/人工校准入口和 UI/治理面 benchmark 摘要 | 这是项目 `1+1>2` 的核心价值主张，当前已能解释、建议调优、沉淀真实样本、做同任务后验对比、导出固定任务契约、执行端到端跑分并解释维度差异，但仍需要更可信的裁判校准和可运营呈现来证明质量与速度收益 |
...
- LLM 裁判与人工校准入口、UI/治理面 benchmark 摘要继续留在 `智能路由收益与切换体感治理` 后续 P1。
