...
|---|---|---|---|---|
| 1 | 智能路由收益与切换体感治理 | P1-主路径易用性 | 在第一版 `ctr eval --input` 离线评测入口之后，继续补自动模型调用执行、更多任务样本和更严格 rubric | 这是项目 `1+1>2` 的核心价值主张，当前已能解释、建议调优、沉淀真实样本、做同任务后验对比并执行手工结果文件评测，但仍需要端到端可重复跑分来证明质量与速度收益 |
