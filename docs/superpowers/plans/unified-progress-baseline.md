...
| 同模型多源池化与注册调度 | 复审新增持续演进特性 | in_progress | P2-能力扩展 | 已新增 `Registration.models` model pool contract：同一 logical model 可编译为多个 registration endpoint，支持 priority 策略、active endpoint、endpoint 启停、upstream service 关联和 warning，并通过 `/api/models/compiled` / `/api/models/compiled/preview` / `/ui` compiled models 暴露；priority active endpoint 已可编译为内部 `registration__*` provider，logical model id 可解析到 active endpoint，并在治理 trace 中写入 `model_pool:<modelId>:<endpointId>`。当前闭环结论是“池化边界已从摘要展示推进到可编译、可测试、可观测、可运行时解析的最小模型池，但 fallback-on-error、health-aware routing、熔断和延迟窗口仍未接管真实请求路径” | `docs/superpowers/plans/2026-04-25-project-goal-user-review-implementation.md` ; `docs/superpowers/specs/2026-04-17-deployment-and-remote-access-design.md` |
...
- `同模型多源池化与注册调度`
  ...
  - 仍需从当前 priority 选择器继续扩展到 fallback-on-error、health-aware routing、熔断、冷却和延迟窗口。
