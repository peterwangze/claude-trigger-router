...
- `智能路由收益与切换体感治理`
  ...
  - 仍需继续补真实质量/失败样本，让 UI / API 能回答“多模型组合到底在哪些任务上提升了质量或速度”。
- `服务端 API key 与鉴权控制面`
...
|---|---|---|---|---|
| 1 | 智能路由收益与切换体感治理 | P1-主路径易用性 | 在已建立 outcome scorecard 与 routing tuning 入口后，继续补真实质量/失败样本 | 这是项目 `1+1>2` 的核心价值主张，当前已能解释和建议调优，但仍需要真实任务证据来证明质量与速度收益 |
