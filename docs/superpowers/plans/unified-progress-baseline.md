...
| 同模型多源池化与注册调度 | 复审新增持续演进特性 | in_progress | P2-能力扩展 | 已新增 `Registration.models` 编译期 model pool contract：同一 logical model 可编译为多个 registration endpoint，支持 priority 策略、active endpoint、endpoint 启停、upstream service 关联和 warning，并通过 `/api/models/compiled` / `/api/models/compiled/preview` / `/ui` compiled models 暴露。当前闭环结论是“池化边界已从摘要展示推进到可编译、可测试、可观测的最小模型池，但运行时 fallback、health-aware routing、熔断、延迟窗口和 trace 调度原因仍未接管真实请求路径” | `docs/superpowers/plans/2026-04-25-project-goal-user-review-implementation.md` ; `docs/superpowers/specs/2026-04-17-deployment-and-remote-access-design.md` |
...
- `同模型多源池化与注册调度`
  - `Registration.models` 与 `Registration.upstream_services` 已形成最小编译期 model pool，`Registration.models` 中相同 `id` 的多个 endpoint 会进入同一 logical model pool，并暴露 priority active endpoint、启停状态、upstream service 关联和 warning。
  - 需要把当前 priority compile contract 继续推进到运行时调度，并从保守策略扩展到 round-robin / health-aware / fallback-on-error。
