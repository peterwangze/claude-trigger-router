...
> 项目已经完成治理主链首轮能力落地，v1.3.0 基础路由常用体验、v1.4.0 SmartRouter 常用体验、v1.5.0 入口基础功能稳定、v1.6.0 多模型收益运营化、v1.7.0 远程服务 / 模型池安全体验、v1.8.0 低侵入 agent/tool 增强与架构减压、v1.9.0 用户入口与远程客户端一致性收口、v1.10.0 智能路由自适应与多模型协同增强、v1.11.0 基础路由流式稳定性首轮止血，以及 v1.12.0 流式传输韧性与远程中转稳定性修复已阶段闭环。2026-06-05 继续从用户易用性和使用体验复审后，确认下一轮应优先推进 v1.13.0 核心路由用户体感与看护补强；完成路由预演、基础路由触发解释、SmartRouter 起步模板收口、协作口径校准和用户体感 E2E 后，再回到配置产品化最终收口与 CLI/setup UX 重设计。入口稳定、基础路由即时流式输出、上游中途断流可读错误、远程中转取消上游、结构化 API error、发布看护、UI 基础交互、收益证据链、远程服务安全、受保护 UI admin 入口和 agent/tool trace contract 继续作为回归底线。
...
| 核心路由用户体感与看护补强 | 用户高频核心主线 | in_progress | P1-主路径易用性 | 2026-06-05 从用户易用性和使用体验继续深度复审后新增为 v1.13.0 主线：近期 P0/P1 问题说明此前看护偏工程内部，缺少真实用户视角的“慢、卡、错路由、错误不可读、入口模板过重”拦截。当前闭环目标是“新增 route preview 让用户能预判基础路由和 SmartRouter 选择；收口基础路由触发解释与 SmartRouter 起步模板；校准协作模式不默认并发执行的真实能力；补首包、持续输出、上游断流、远程中转和结构化错误的用户体感 E2E”。 | `docs/superpowers/plans/2026-05-07-core-routing-version-plan.md` ; `docs/superpowers/plans/progress-issue-log.md` |
...
|---|---|---|---|---|
| 1 | 核心路由用户体感与看护补强 | P1-主路径易用性 | 推进 v1.13.0：route preview、基础路由触发解释、SmartRouter 起步模板、协作口径校准和用户体感 E2E | 近期真实问题集中在基础路由和 SmartRouter 的强感知体验，说明必须先把“慢、卡、错路由、错误不可读、入口过重”的用户视角看护补齐 |
| 2 | 配置产品化最终收口 | P1-主路径易用性 | 继续服务入口主路径：把 `id/api/key/interface/model/thinking/metadata`、路由槽位和 capability warning 继续收敛进 README / configuration guide / UI / setup | v1.13.0 之后回到配置心智统一；配置心智不统一仍会直接破坏 setup、doctor、UI save 和日常修配置体验 |
...
| 4 | 项目目标与用户使用视角复审 | P1-主路径易用性 | 继续作为校准主线归档新增事项和顺序调整，尤其看护基础路由流式输出、上游中途断流、远程中转取消、结构化 API error 和发布门禁 | v1.11.0/v1.12.0 说明 closed 事项仍可能暴露真实主路径回归，复审必须持续把低频扩展让位给用户高频基础能力 |
...
| 8 | 治理观测增强 / 运营化 | P2-能力扩展与体验增强 | 在已接入 routing outcome、benchmark history、pool health、key audit、v1.11.0 流式旁路观测和 v1.12.0 上游断流 error event 的基础上继续压低维护者排障成本 | 治理观测需要服务入口稳定，而不是只服务高级维护者能力 |
| 9 | 部署形态与远程接入收敛 | P2-能力扩展与体验增强 | 在 v1.9.0 修正远程客户端入口叙事后，继续补服务发现和更完整服务模式 | 远程状态查询、模型调用转发和远端注册摘要已阶段闭环，后续可建立在准确文档和 setup 心智上继续演进 |
| 10 | CLI 稳定性与发布工程 | P3-治理支撑 | 持续补 packaged CLI 用户流 E2E 与 release verify slice，优先覆盖当前发布主路径 | 它需要持续跟随入口主路径一起扩 coverage；当门禁不稳定、基础路由流式或结构化 API error 退化时，按 P1/P0 触发条件前置处理 |
| 11 | 已闭环事项复审校准 | P3-治理支撑 | 持续复审 closed 事项，并把新发现的问题承接为增量事项 | 避免历史 closed 结论与新入口稳定目标漂移 |
| 12 | 进展文档体系治理 | P3-治理支撑 | 持续维护统一基线、优先级、闭环标准和执行顺序表 | 它是总台账机制，必须持续维护，但不抢占主功能闭环 |
| 13 | 问题修改记录 | P3-治理支撑 | 发现治理偏差或真实主路径回归就即时追加 issue log 并更新闭环结论 | 它是防重复踩坑机制，属于全程伴随动作 |
...
执行规则：后续用户只说“按照计划优先级继续推进”时，默认先按上述版本计划推进；当前先闭环 v1.13.0 核心路由用户体感与看护补强。若出现安全风险、P0 主路径故障，或基础路由即时流式输出 / 上游中途断流可读错误 / 远程中转取消上游 / 结构化 API error / fresh setup / 发布门禁 / 远程转发 / 配置保存 / 收益证据链 / 远程服务安全 / 模型池调度 / 受保护 UI admin 入口 / agent-tool trace contract 再次退化，则重新前置处理。
