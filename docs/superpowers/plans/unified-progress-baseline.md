...
- `基础路由与 SmartRouter 常用体验收口`
  ...
  - v1.3.0 需要继续补基础路由体验：基础路由打包后 smoke；基础路由五槽位配置向导与 `config/trigger.routing.yaml` 可复制模板已闭环，`ctr doctor` 槽位体检已闭环，`/ui` 使用者工作台也已展示 default/think/longContext/background/webSearch 槽位解析、上游目标、能力、warning 和 context window 可操作提示。
...
|---|---|---|---|---|
| 1 | 基础路由与 SmartRouter 常用体验收口 | P1-主路径易用性 | v1.3.0 已补基础路由五槽位模板、`ctr doctor` 槽位体检、`/ui` 路由解释和 context window 可操作提示；下一步继续补基础路由 smoke | 这是用户每天实际使用的最高频路径；不先把 `Router.default/think/longContext/background/webSearch` 与 SmartRouter 配置解释收口，后续评测和服务化都会缺少稳定用户入口 |
