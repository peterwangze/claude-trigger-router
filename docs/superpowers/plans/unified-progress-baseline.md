...
- `服务端 API key 与鉴权控制面`
  ...
  - 已补 request/token quota 的进程内执行、超额 429、审计告警，以及 read-only 对健康/状态 GET 接口的最小权限边界。
