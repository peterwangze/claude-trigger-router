...
| UI 双层工作台收敛 | 持续演进特性 | in_progress | P2-能力扩展 | 当前 `/ui` 已完成首轮使用者 / 维护者 surface 分层：使用者默认进入配置草稿、模型、路由、compiled preview 与保存动作，维护者独立承接 trace、metrics、异常阈值、快照和归档；2026-05-23 架构复审确认 `src/ui/workbench.ts` 仍是大型 HTML/CSS/内联 JS 字符串；2026-06-05 的 v1.16.0 用户视角复审已先补齐角色化 UI 入口与设计辅助面板，第一屏已区分本地使用者、远程客户端、服务维护者和路由设计辅助，并把角色入口纳入 fragment contract 与 DOM 跳转看护。2026-06-06 已自动安装 `figma-create-design-system-rules`、`figma-generate-design`、`figma-implement-design` 三个 Codex/Figma 辅助 skill，并把角色化 UI 体验设计、设计系统规则、任务流和真实浏览器验收追加到 v1.17.0。当前闭环结论是“UI 双层产品边界已有可操作入口，但 v1.17.0 不能只继续工程拆分；必须把角色化视觉系统、信息架构、组件状态和实现 contract 一起收口，再进入真实浏览器 smoke”。 | `docs/superpowers/specs/2026-04-17-dual-surface-ui-ux-design.md` ; `docs/superpowers/plans/2026-04-17-dual-surface-ui-ux-implementation.md` ; `docs/superpowers/specs/2026-04-10-cli-setup-ux-redesign-design.md` ; `docs/superpowers/plans/2026-05-07-core-routing-version-plan.md` ; `docs/superpowers/plans/progress-issue-log.md` |
...
| 5 | UI 双层工作台收敛 | P2-能力扩展与体验增强 | 推进 v1.17.0：基于 v1.16.0 已补的角色化第一屏、UX 诊断面板和角色入口 DOM 跳转看护，继续拆分使用者/维护者渲染片段、CSS/JS helper、trace span 视图；新增角色化 UI 体验设计、Codex/Figma 辅助 skill 接入、设计系统规则和真实浏览器 smoke | UI 已是用户入口的一部分，v1.9.0 已处理 admin 鉴权入口；本轮反馈说明 UI 不能只“功能可达”，还必须让本地使用者、远程客户端、服务维护者和路由设计者按各自任务路径顺滑进入 |
...
执行规则：后续用户只说“按照计划优先级继续推进”时，默认先按上述版本计划推进；v1.16.0 已闭环后，当前默认从 v1.17.0 UI 双层工作台收敛开始，并优先承接角色化 UI 体验设计、辅助 skill 接入和真实浏览器 smoke。若出现安全风险、P0 主路径故障，或基础路由即时流式输出 / 上游中途断流可读错误 / 远程中转取消上游 / 结构化 API error / route preview / fresh setup / 发布门禁 / 远程转发 / 配置保存 / 收益证据链 / 远程服务安全 / 模型池调度 / 受保护 UI admin 入口 / 角色化 UI 入口 / 角色化 UI 体验设计 / agent-tool trace contract 再次退化，则重新前置处理。
