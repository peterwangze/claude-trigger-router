...
| 基础路由与 SmartRouter 常用体验收口 | 用户高频核心主线 | planned | P1-主路径易用性 | 2026-05-07 从用户使用频率复审后新增为后续版本主线：基础路由 `Router.default/think/longContext/background/webSearch` 和 SmartRouter 规则/候选/切换体感是用户每天最常用的核心功能，优先级应高于离线评测、benchmark 看板、远程部署和 agent/tool 扩展。当前闭环结论是“运行时能力已经具备，但用户侧配置向导、doctor/UI 路由解释、context window 配置提示、SmartRouter 模板、路由决策解释和常用路径打包后 smoke 仍需作为 v1.3.0/v1.4.0 主线收口”。 | `docs/superpowers/plans/2026-05-07-core-routing-version-plan.md` |
...
- `基础路由与 SmartRouter 常用体验收口`
  ...
  - v1.3.0 需要继续补基础路由体验：context window UI 可操作提示、基础路由打包后 smoke；基础路由五槽位配置向导与 `config/trigger.routing.yaml` 可复制模板已闭环，`ctr doctor` 槽位体检已闭环，`/ui` 使用者工作台也已展示 default/think/longContext/background/webSearch 槽位解析、上游目标、能力和 warning。
...
|---|---|---|---|---|
| 1 | 基础路由与 SmartRouter 常用体验收口 | P1-主路径易用性 | v1.3.0 已补基础路由五槽位模板、`ctr doctor` 槽位体检和 `/ui` 路由解释；下一步继续补 context window UI 提示和基础路由 smoke | 这是用户每天实际使用的最高频路径；不先把 `Router.default/think/longContext/background/webSearch` 与 SmartRouter 配置解释收口，后续评测和服务化都会缺少稳定用户入口 |
