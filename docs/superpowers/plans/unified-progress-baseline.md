...
|---|---|---|---|---|
| 1 | 入口基础功能稳定与易用性巩固 | P1-主路径易用性 | v1.5.0 优先保护 `setup/start/status/code/doctor/ui`、配置保存/修复/迁移、UI 基础交互、coverage 口径和 release verify 入口门禁；2026-05-18 后先补 packaged CLI e2e 超时定位、残留清理和可诊断日志 | 这是用户每天最先触碰的入口；入口不稳或发布门禁不可依赖时，benchmark、服务化和模型池收益都无法兑现 |
