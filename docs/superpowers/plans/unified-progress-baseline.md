...
| 治理观测增强 / 运营化 | 持续演进特性 | in_progress | P2-能力扩展 | metrics、快照、归档、异常检测、时间窗趋势和治理健康摘要已具备第一轮基础，当前闭环结论是“已从可调试进入可运营增强阶段，维护者能通过 `health` 摘要判断 idle / healthy / watch / critical，README / configuration guide / CLI test matrix 已同步健康摘要口径；后续仍需继续收口 health/action 与 trace detail 的联动、真实浏览器 smoke 和发布门禁切片”；输入侧优化将作为治理增强链路的下一轮设计输入推进 | `docs/superpowers/plans/2026-04-04-router-evolution-tracker.md` ; `docs/superpowers/plans/2026-04-06-router-progress-calibration.md` ; `docs/superpowers/specs/2026-04-17-governance-input-optimization-design.md` |
