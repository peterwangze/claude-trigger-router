...
| Trigger 收编到 SmartRouter（统一路由引擎化） | 持续演进特性 | in_progress | P1-主路径易用性 | 已完成目标纠偏并推进到第二阶段：SmartRouter 新 contract 已进入类型、配置归一、selector 主入口、主请求链与响应链的部分桥接；当前闭环结论是“逻辑主线已实质启动，但 Trigger 完全内收、治理默认能力完全内收以及 trace/diagnostics 统一仍未完成；对外口径收口继续后置” | `docs/superpowers/plans/2026-04-15-trigger-smart-router-consolidation.md` |
