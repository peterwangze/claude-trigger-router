...
- `同模型多源池化与注册调度`
  ...
  - 仍需从当前 priority + fallback-on-error 继续扩展到 health-aware routing、熔断、冷却和延迟窗口。
