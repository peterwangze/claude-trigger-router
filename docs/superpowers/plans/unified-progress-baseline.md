...
> 项目已经完成治理主链首轮能力落地，当前进入“基础路由与 SmartRouter 常用体验优先收口、治理观测增强、配置产品化、CLI / setup UX 与发布工程持续收口、部署形态收敛与 UI 双层工作台设计并行推进”阶段。
...
| 基础路由与 SmartRouter 常用体验收口 | 用户高频核心主线 | in_progress | P1-主路径易用性 | 2026-05-07 从用户使用频率复审后新增为后续版本主线：基础路由 `Router.default/think/longContext/background/webSearch` 和 SmartRouter 规则/候选/切换体感是用户每天最常用的核心功能，优先级应高于离线评测、benchmark 看板、远程部署和 agent/tool 扩展。当前闭环结论是“v1.3.0 基础路由常用体验已阶段闭环；v1.4.0 已闭环 SmartRouter 常用规则模板、UI 配置解释、近期路由决策摘要和切换体感摘要：新增 `config/trigger.smart-router.yaml` 覆盖 coding/review/architecture/long_context/fast_reply，`/api/models/compiled` 和 preview 已返回 SmartRouter explanation，`/ui` 可展示规则顺序、候选、router_model、semantic/sticky 与 fallback；governance trace 现在记录 route source、rule、confidence 和选中模型，`/api/governance/traces` 与 `/ui` 可把最近请求的 rule / semantic intent / fallback reason 翻译成可读摘要；switch continuity summary 已把 initial/final model、sticky、alignment、cascade 和 route source 合成为 stable/aligned/watch/critical 等可读状态；下一步继续收口候选模型配置向导和慢路由/错路由调优建议”。 | `docs/superpowers/plans/2026-05-07-core-routing-version-plan.md` |
...
3. 配置产品化最终收口
4. CLI / setup UX 重设计
5. 服务端 API key 与鉴权控制面
6. server/cloud 一键部署与角色化运维入口
7. 项目目标与用户使用视角复审
...
- `基础路由与 SmartRouter 常用体验收口`
  ...
  - v1.4.0 已闭环 SmartRouter 规则模板、配置解释、近期路由决策摘要和切换体感摘要；仍需继续补候选模型配置向导和慢路由/错路由调优建议。
- `智能路由收益与切换体感治理`
...
|---|---|---|---|---|
| 1 | 基础路由与 SmartRouter 常用体验收口 | P1-主路径易用性 | v1.3.0 基础路由已阶段闭环；v1.4.0 已闭环 SmartRouter 规则模板、配置解释、近期路由决策摘要和切换体感摘要，下一步优先补候选模型配置向导和慢路由/错路由调优建议 | 这是用户每天实际使用的最高频路径；基础路由 smoke 已补齐，后续应把 SmartRouter 常用体验收口成同样可配置、可解释、可回退的用户入口 |
| 2 | 智能路由收益与切换体感治理 | P1-主路径易用性 | v1.4.0 继续围绕 SmartRouter 调优建议推进；benchmark 历史看板和人工校准表单后移到 v1.5.0 | 它仍是 `1+1>2` 的价值证明，但 `ctr eval` 是验证工具，不应再压过基础路由与 SmartRouter 常用体验 |
...
| 5 | 服务端 API key 与鉴权控制面 | P1-主路径易用性 | 继续补服务端部署默认安全策略、密钥轮换和维护手册 | server/cloud 和池化会放大资源泄漏风险，但从本地日常使用频率看应排在核心路由体验之后 |
| 6 | server/cloud 一键部署与角色化运维入口 | P1-主路径易用性 | 在安全默认值基础上补一键部署脚本/命令和角色化指导 | 一键部署必须建立在鉴权控制面之后，且不能抢占本地核心路由体验 |
| 7 | 项目目标与用户使用视角复审 | P1-主路径易用性 | 继续作为校准主线归档新增事项和顺序调整 | 本轮复审明确了“用户高频核心路由优先”的排序，后续仍需防止主线再次偏向低频治理能力 |
| 8 | 同模型多源池化与注册调度 | P2-能力扩展与体验增强 | 在 logical model pool、endpoint health、fallback、熔断、延迟窗口、持久化 health 和维护者运营视图基础上补主动探测、成本/速率元数据与更多策略 | 它能提升服务质量，但依赖鉴权、部署和注册边界稳定后再做 |
| 9 | 部署形态与远程接入收敛 | P2-能力扩展与体验增强 | 在最小远端请求转发与只读注册摘要同步之后，继续补服务发现和更完整服务模式 | 远程状态查询、模型调用转发和远端注册摘要已阶段闭环，下一步应建立在安全部署和池化调度基础上 |
| 10 | UI 双层工作台收敛 | P2-能力扩展与体验增强 | 先承载基础路由解释和 SmartRouter 决策解释，再承载收益、鉴权、部署、池化和维护者观测入口 | UI 后续首要任务是让高频路由配置和决策可理解，而不只是继续扩维护者观测 |
| 11 | 治理观测增强 / 运营化 | P2-能力扩展与体验增强 | 把基础路由、SmartRouter 切换体感、pool health、key audit 纳入 health/metrics | 治理观测需要跟随核心价值和服务化风险扩展，而不只是 trace 查看 |
...
| 13 | CLI 稳定性与发布工程 | P3-治理支撑 | 持续补 packaged CLI 用户流 E2E 与 release verify slice，优先覆盖 v1.3.0/v1.4.0 核心路由用户流 | 它需要持续跟随前面新路径一起扩 coverage，属于伴随式治理支撑 |
| 14 | 已闭环事项复审校准 | P3-治理支撑 | 持续复审 closed 事项，并把新发现的问题承接为增量事项 | 避免历史 closed 结论与新路由/服务化目标漂移 |
...
| v1.5.0 | 多模型组合收益运营化 | 维护者能用固定样本和真实 trace 判断路由配置是否真的提升质量/速度 |
| v1.6.0 | 远程服务与模型池安全体验 | 服务提供者能安全暴露服务，远程使用者能稳定接入，模型池提升可用性而不放大风险 |
| v1.7.0 | 低侵入 agent/tool 增强 | 增强能力进入现有路由与治理体系，不扩张成平行 agent 平台 |

执行规则：后续用户只说“按照计划优先级继续推进”时，默认先按上述版本计划推进；除非出现阻塞发布、安全风险或 P0 主路径故障，不再优先扩展 benchmark、远程部署、模型池或 agent/tool 能力。
...
- 人工校准 UI 表单和 benchmark 历史看板继续留在 `智能路由收益与切换体感治理` 后续 P1，并按新版本计划后移到 v1.5.0。
...
- SmartRouter 规则模板、候选模型配置向导、路由决策解释、sticky/alignment 切换体感和慢路由/错路由调优建议进入 v1.4.0。
- benchmark 历史看板、人工校准 UI 表单和评测/真实 trace 的统一解释进入 v1.5.0。
- 服务端安全默认值、密钥轮换手册、server/cloud 一键部署、主动 pool health、成本/速率元数据和更多调度策略继续按 v1.6.0 服务化与模型池安全体验推进。
