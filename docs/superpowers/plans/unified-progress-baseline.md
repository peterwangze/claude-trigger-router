...
- `智能路由收益与切换体感治理`
  ...
  - 仍需继续补内置 LLM 裁判执行器、人工校准 UI 表单和 benchmark 历史看板，让同任务多模型 A/B 从“端到端可重复跑分”推进到“可运营的收益评估”。
- `服务端 API key 与鉴权控制面`
...
|---|---|---|---|---|
| 1 | 智能路由收益与切换体感治理 | P1-主路径易用性 | 在 `ctr eval` 校准字段与 UI benchmark summary 之后，继续补内置 LLM 裁判执行器、人工校准表单和 benchmark 历史看板 | 这是项目 `1+1>2` 的核心价值主张，当前已能解释、建议调优、沉淀真实样本、做同任务后验对比、导出固定任务契约、执行端到端跑分、解释维度差异，并用人工/外部裁判分数校准 rubric；后续需要把裁判执行和历史运营视图做成更完整的维护者流程 |
...
- 内置 LLM 裁判执行器、人工校准 UI 表单和 benchmark 历史看板继续留在 `智能路由收益与切换体感治理` 后续 P1。
