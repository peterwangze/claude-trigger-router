...
| Trigger 收编到 SmartRouter（统一路由引擎化） | 持续演进特性 | in_progress | P1-主路径易用性 | 已完成目标纠偏并开始落第一轮核心逻辑：SmartRouter 新 contract 已进入类型、配置归一与 selector 主入口；当前闭环结论是“逻辑主线已启动，但 Trigger 完全内收、治理默认能力完全内收以及 trace/diagnostics 统一仍未完成；对外口径收口继续后置” | `docs/superpowers/plans/2026-04-15-trigger-smart-router-consolidation.md` |
