...
- 已启动第一阶段执行：README 与 setup 完成页开始收口为统一 Router 叙事
- setup 下一步提示已改为“显式规则 / 智能兜底”表达，不再把 TriggerRouter / SmartRouter 作为默认用户入口术语

当前未完成：

- README 仍需继续从分节表达收口到统一 Router 主表达
- example config 与 `/ui` 主文案仍未进入本轮执行
- 尚未建立这一主线的专门回归测试组
