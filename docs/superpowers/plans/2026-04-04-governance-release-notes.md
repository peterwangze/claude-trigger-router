...
- governance trace 归档支持管理 API，可按日期检索并查看/删除历史归档
- anomaly 阈值已接入正式配置文件，并同步到 `/ui` 调参面板默认值
- 定时快照调度支持本地持久化与重启恢复，周期观测不再只依赖单次进程生命周期
- governance archive 默认启用 gzip 压缩，并支持分页浏览归档列表
- `/ui` anomaly tuning 支持直接写回配置文件，无需手动编辑 YAML/JSON
- `/ui` 新增快照/归档管理面板，可创建快照并查看 exports / schedules / archives 状态
- 管理面板已支持按日期和分页参数浏览归档列表，便于较长历史窗口下运维排查
...
- 当前归档管理仍基于文件名和日期过滤，尚未支持全文检索、压缩预览或分页浏览
- `/ui` 调参当前只覆盖本次页面查询，尚未支持直接写回配置文件
- 调度恢复当前基于本地 schedule 文件，尚未支持分布式锁或多实例协调
- 归档分页当前只支持列表级分页，尚未提供归档内容内分页或在线解压预览
- anomaly tuning 当前只覆盖部分核心阈值字段，尚未开放全部 warn/critical 参数的可视化编辑
- 当前管理面板仍是轻量版，尚未支持归档删除、详情查看和调度停用等交互按钮
