...
| PI-031 | 同一 session 中断后用 resume 恢复明显变慢或卡住 | 2026-06-15 | closed | 已通过 v1.20.1 阶段闭环：`resume` 长历史首包前路径新增 preflight diagnostics，SmartRouter full conversation 分析有 bounded recent-window 与字符预算，sticky correction 在同 session fingerprint 稳定时先于 semantic classifier/SmartRouter fallback 复用模型，context window token count 具备请求签名缓存诊断，alignment summary 使用 bounded context、独立 timeout 和 skip reason；新增 `npm run test:resume-stability` 并纳入本地/GitHub 发布门禁。若 resume 长历史恢复再次明显慢于正常任务或无解释卡住，按 P0 重新前置。 | `src/index.ts` ; `src/router/index.ts` ; `src/trigger/analyzer.ts` ; `src/trigger/selector.ts` ; `src/governance/context-alignment.ts` ; `docs/superpowers/plans/2026-05-07-core-routing-version-plan.md` ; `docs/release-notes-v1.20.1.md` |
| PI-032 | API 报错后继续或 resume 后继续仍明显慢卡 | 2026-06-17 | closed | 已通过 v1.20.2 阶段闭环：`ContextAnalyzer.analyzeWithBudget()` 不再为了精确 `originalChars` 重建完整 full conversation，改为估算并标记；router token count cache 改用紧凑签名，不再在 cache 命中前序列化完整长历史；SmartRouter fallback / semantic classifier 默认使用 30000ms preflight 短超时，并支持配置覆盖。若 API 报错后继续或 resume 后继续再次明显慢卡，按 P0 重新前置。 | `src/trigger/analyzer.ts` ; `src/router/index.ts` ; `src/trigger/selector.ts` ; `src/governance/semantic-router.ts` ; `docs/superpowers/plans/2026-05-07-core-routing-version-plan.md` ; `docs/release-notes-v1.20.2.md` |
...
- 当前状态：`closed`
- 闭环结论：该问题不回退 PI-026 的第一屏角色入口阶段闭环；它已通过 v1.17.0 阶段闭环：角色/任务流、信息架构、设计 token、组件状态、实现 contract、真实浏览器 smoke 和 v1.17.0 发布验收清单均已落入版本计划，且没有新增平行 UI 状态。后续若 `/ui` 角色入口、trace evidence、移动/桌面布局或 browser smoke 再次退化，按 P1/P2 重新前置。
- 关联文档：
  - `docs/superpowers/plans/2026-05-07-core-routing-version-plan.md`
  - `docs/superpowers/plans/unified-progress-baseline.md`
...
- 当前状态：`closed`
- 闭环结论：该问题不回退 PI-027 / v1.17.0 的阶段闭环；它已通过 v1.18.0 阶段闭环：维护者工作台新增 decision rail，把 Operations risk、Guardrail summary 和 Outcome scorecard 汇总为顶部扫描路径；视觉规则继续保持低噪声运营工具密度，桌面三列、移动单列，并已通过 DOM/style/browser smoke 看护。后续若 Web UI 角色任务流、decision rail、桌面/移动布局或 browser smoke 再次退化，按 P1/P2 重新前置。
- 关联文档：
  - `docs/superpowers/plans/2026-05-07-core-routing-version-plan.md`
  - `docs/superpowers/plans/unified-progress-baseline.md`
...
### PI-030：断流频发涉及多种常见场景，需要全量稳定性与可用性复审

- 发现时间：2026-06-12
- 严重级别：P0
- 现象：用户反馈最近断流问题频发，且涉及各种用户常见场景。v1.19.3 已修复 stream lifecycle、默认流式安全关闭、远程/agent 取消传播和第二轮 signal 隔离，但继续只做单点补丁不足以覆盖本地直连、远程中转、agent/tool 续写、手动停止、错误后继续、第二轮会话、CLI 启动和发布门禁的组合风险。
- 影响范围：
  - 工具调用续写时 `rewriteStream()` 的控制器异常或下游取消可能继续表现为 socket-level 中断
  - 上游/代理返回空 body、非 SSE body 或异常结束时，用户可能只看到卡住或断开
...
### PI-031：同一 session 中断后用 resume 恢复明显变慢或卡住

- 发现时间：2026-06-15
- 严重级别：P0/P1
- 现象：用户反馈同一个 session 的任务中断退出后重启，并用 `resume` 恢复时，相比正常新任务会非常慢，甚至直接卡住。
- 初步定位：
  - `resume` 恢复路径通常会携带同一 session 的完整历史消息，CTR 仍会在 `/v1/messages` 首包前同步执行 SmartRouter、基础路由、上下文窗口估算和治理前置逻辑。
  - `src/router/index.ts` 的 `calculateTokenCount()` 会遍历全部 `messages/system/tools` 并用 tiktoken 编码；长历史、工具结果和大 schema 会把前置 CPU 成本放大。
  - `src/trigger/analyzer.ts` 在 `analysis_scope=full_conversation` 时会拼接全部用户消息；随后 `src/trigger/selector.ts` 会基于该长文本做规则、semantic classifier、SmartRouter LLM fallback、sticky fingerprint 和 routing hint。
  - 如果同 session 上一次成功模型与本轮选择模型不同，`src/index.ts` 会调用 `contextAlignmentService.summarizeTransition()`，在首包前额外发起 loopback LLM 摘要请求；长 `analyzedText` 会进一步放大等待。
  - 这与 PI-029 的“第二轮 signal 隔离/断流”相邻但不是同一类问题：当前更像 resume 长历史在路由/治理前置阶段缺少预算、度量和快速路径。
- 影响范围：
  - Claude/Codex `resume` 恢复长任务
  - 同一 session 中断后继续执行、错误后继续执行和长工具链历史恢复
  - SmartRouter `full_conversation`、semantic classifier、SmartRouter fallback、sticky alignment、context window guard 等首包前路径
- 修正动作：
  - 已通过 v1.20.1 `resume` 恢复性能专项闭环。
  - 已补可观测性：记录 preflight route analysis、token count、alignment summary 的耗时、输入字符数和消息数，并在 trace span 中按 request/session 暴露卡点。
  - 已补性能预算：`analysis_scope=full_conversation` 进入 SmartRouter/semantic/fingerprint 前受 bounded recent-window 与字符预算保护，并记录截断 reason。
  - 已补快速路径：同 session resume 若 last task fingerprint 未明显变化，优先复用 sticky model，避免无必要的 semantic classifier、SmartRouter fallback 和 alignment loopback。
  - 已补回归门禁：新增 `npm run test:resume-stability`，覆盖长消息、工具结果、`full_conversation`、SmartRouter fallback、alignment enabled、context window guard 和同 session 第二轮恢复相关路径。
- 当前状态：`closed`
- 闭环结论：该问题已从“泛化第二轮卡顿”拆出为独立 resume 长历史性能问题，并通过 v1.20.1 阶段闭环；PI-029/PI-030 的 closed 结论仍只覆盖 stream/signal/断流类问题，后续若 `resume` 恢复再次明显慢于正常任务或无解释卡住，按 P0 重新前置。
- 关联文档：
  - `src/index.ts`
  - `src/router/index.ts`
  - `src/trigger/analyzer.ts`
  - `src/trigger/selector.ts`
