...
export function renderWorkbenchHtml(rawInitialConfig: any, configuredThresholds: any = {}): string {
  ...
  return (
    ...
    `<div class="home-flow" aria-label="本地路由配置流程">` +
    `<div class="home-flow-step active"><span>1</span><strong>基础配置</strong><small>模型与 Key</small></div>` +
    `<div class="home-flow-step"><span>2</span><strong>智能路由</strong><small>初始化策略</small></div>` +
    `<div class="home-flow-step"><span>3</span><strong>规则管理</strong><small>新增修改删除</small></div>` +
    `<div class="home-flow-step"><span>4</span><strong>预览保存</strong><small>确认后生效</small></div>` +
    `</div>` +
    `<div class="panel quick-config-panel">` +
    `<div class="surface-heading stacked-heading"><strong>基础配置</strong><span class="muted">先接入一个可用模型，再决定默认路由和智能路由策略。</span></div>` +
    `<div class="quick-config-grid">` +
    `<div class="quick-config-main">` +
    `<div class="row"><strong>模型接入</strong><span id="quickConfigStatus" class="muted">等待载入当前配置</span></div>` +
    ...
    `</div>` +
    `<div class="home-router-cards">` +
    `<section class="home-router-card" id="homeSmartRouterCard" aria-label="智能路由初始化">` +
    `<div class="home-card-head"><div><strong>智能路由</strong><span class="muted" id="homeSmartRouterMeta">等待载入 SmartRouter 状态</span></div><span id="homeSmartRouterState" class="pill info">checking</span></div>` +
    `<div class="home-router-actions">` +
    `<button id="initializeSmartRouterBtn" type="button">初始化</button>` +
    `<button id="addHomeRouteRuleBtn" class="secondary" type="button">新增规则</button>` +
    `<button id="addHomeSmartCandidateBtn" class="secondary" type="button">新增候选</button>` +
    `<button id="clearSmartRouterBtn" class="secondary" type="button">清空路由</button>` +
    `</div>` +
    `</section>` +
    `<section class="home-router-card" aria-label="路由规则摘要">` +
    `<div class="home-card-head"><div><strong>路由规则</strong><span class="muted">常用规则在首页直接管理，复杂字段进入高级编辑。</span></div><button id="openRouteDesignerBtn" class="secondary compact-btn" type="button">编辑</button></div>` +
    `<div id="homeRouteRuleList" class="home-route-list"><span class="muted">尚未载入规则</span></div>` +
    `</section>` +
    `<section class="home-router-card" aria-label="候选模型摘要">` +
    `<div class="home-card-head"><div><strong>候选模型</strong><span class="muted">SmartRouter 会在候选中选择最合适的模型。</span></div></div>` +
    `<div id="homeSmartCandidateList" class="home-route-list"><span class="muted">尚未载入候选</span></div>` +
    `</section>` +
    `</div>` +
    ...
    `</section>` +
    `<section class="config-live-panel route-visual-panel" aria-label="路由路径">` +
    `<div class="row"><strong>路由渲染示意图</strong><span id="routePathHint" class="muted">展示当前路径与模板变更</span></div>` +
    ...
    `const homeSmartRouterMeta=document.getElementById('homeSmartRouterMeta');` +
    `const homeSmartRouterState=document.getElementById('homeSmartRouterState');` +
    `const homeRouteRuleList=document.getElementById('homeRouteRuleList');` +
    `const homeSmartCandidateList=document.getElementById('homeSmartCandidateList');` +
    ...
    `function getSmartRouterDraftSummary(config){ const smart=getDraftSmartRouterConfig(config || {}); const rules=Array.isArray(smart?.rules) ? smart.rules : []; const candidates=Array.isArray(smart?.candidates) ? smart.candidates : []; return { smart, rules, candidates, enabled:Boolean(smart?.enabled || rules.length || candidates.length), semantic:Boolean(smart?.semantic?.enabled), sticky:Boolean(smart?.sticky?.enabled || smart?.sticky?.alignment?.enabled) }; }` +
    `function renderHomeSmartRouterSummary(config){ const summary=getSmartRouterDraftSummary(config || getActiveConfig()); const cls=summary.enabled ? 'info' : 'warn'; homeSmartRouterState.className='pill '+cls; homeSmartRouterState.textContent=summary.enabled ? '已启用' : '未启用'; homeSmartRouterMeta.textContent=(summary.rules.length)+' rules · '+(summary.candidates.length)+' candidates · fallback '+(summary.smart?.fallback || 'default'); homeRouteRuleList.innerHTML=summary.rules.length ? summary.rules.map((rule,index)=>'<div class=\"home-route-item\"><span><strong>'+esc(rule.name || ('Rule #'+(index+1)))+'</strong><small>'+esc((rule.model || '-')+' · priority '+(rule.priority ?? 10))+'</small></span><button type=\"button\" class=\"secondary compact-btn\" data-remove-home-rule=\"'+index+'\">删除</button></div>').join('') : '<div class=\"home-route-empty\">未配置规则，默认走 Router.default。</div>'; homeSmartCandidateList.innerHTML=summary.candidates.length ? summary.candidates.map((candidate,index)=>'<div class=\"home-route-item\"><span><strong>'+esc(candidate.model || ('Candidate #'+(index+1)))+'</strong><small>'+esc(candidate.description || '候选模型')+'</small></span><button type=\"button\" class=\"secondary compact-btn\" data-remove-home-candidate=\"'+index+'\">删除</button></div>').join('') : '<div class=\"home-route-empty\">尚未配置候选，初始化后会按模型生成建议。</div>'; }` +
    `function renderRoutePathDiagram(config,options={}){ const active=config || getActiveConfig(); const candidate=options.candidateModel || null; const model=getRouteModel(active,candidate); const router=active?.Router || {}; const exists=Boolean(candidate && getModelById(active,candidate.id)); const stage=candidate ? (exists ? '更新模型' : '新增模型') : '当前路径'; const summary=getSmartRouterDraftSummary(active); const finalId=candidate?.id || router.default || model.id || '-'; const steps=[{label:'入口',title:'用户请求',detail:stage,state:'input'},{label:'代理',title:'CTR '+String(active?.HOST || '127.0.0.1')+':'+String(active?.PORT || '3456'),detail:String(active?.Runtime?.mode || 'local'),state:'proxy'}]; if(summary.enabled){ steps.push({label:'策略',title:'SmartRouter',detail:summary.rules.length+' rules · '+summary.candidates.length+' candidates',state:'smart'}); if(summary.rules.length){ steps.push({label:'规则',title:summary.rules[0].name || 'Rule #1',detail:(summary.rules[0].model || 'model pending')+' · priority '+(summary.rules[0].priority ?? 10),state:'rule'}); } } steps.push({label:'默认',title:'Router.default '+finalId,detail:summary.enabled ? 'fallback '+(summary.smart?.fallback || 'default') : 'direct dispatch',state:candidate ? 'pending' : 'default'}); steps.push({label:'模型',title:(model.id || finalId)+' · '+getProviderLabel(model),detail:model.model || '-',state:candidate ? 'pending' : 'model'}); routePathHint.textContent=candidate ? (stage+'：保存后 Router.default = '+(candidate.id || '-')) : (summary.enabled ? 'SmartRouter 优先匹配规则和候选，未命中时回落 Router.default' : '当前未启用 SmartRouter，请求直接按 Router.default 转发'); routePathDiagram.innerHTML=steps.map((step,index)=>'<div class=\"route-stage route-stage-'+esc(step.state)+'\"><span>'+esc(step.label)+'</span><strong>'+esc(step.title)+'</strong><small>'+esc(step.detail || '-')+'</small></div>'+(index<steps.length-1 ? '<div class=\"route-edge\" aria-hidden=\"true\"><span></span></div>' : '')).join(''); }` +
    ...
    `      const cost=endpoint.cost ? ('cost '+esc(endpoint.cost.inputPer1MTokens ?? '-')+'/'+esc(endpoint.cost.outputPer1MTokens ?? '-')+' '+esc(endpoint.cost.currency || 'USD')+'/1M') : 'cost -';` +
    `      const rate=endpoint.rateLimit ? ('rate '+esc(endpoint.rateLimit.requestsPerMinute ?? '-')+' rpm / '+esc(endpoint.rateLimit.tokensPerMinute ?? '-')+' tpm') : 'rate -';` +
    `      const endpointLabel='<code>'+esc(endpoint.id || '-')+'</code>'+(endpoint.active ? ' <span class="pill info">active</span>' : '')+'<div class="muted">'+esc(endpoint.providerName || '-')+' / '+esc(endpoint.upstreamServiceId || endpoint.upstreamBaseUrl || 'local')+'</div><div class="muted">'+cost+' · '+rate+'</div>';` +
    ...
    `}` +
    `function commitDraftPayload(payload,statusText){ currentDraftConfig=payload; renderModelsForm(payload.Models || []); renderConfigControlForms(payload); draftRouterDefault.value=payload.Router?.default || ''; configDraftEditor.value=JSON.stringify(payload,null,2); renderDraftSummary(payload); renderDraftValidation([],[]); renderCapabilityWarnings(); renderRouterSlotExplanation(withDraftCompiledData(payload)); renderContextWindowGuide(withDraftCompiledData(payload)); renderDraftPreviewMeta(); renderCurrentConfigView(payload); if(statusText){ draftPreviewStatus.textContent=statusText; quickConfigStatus.textContent=statusText; } }` +
    `function initializeSmartRouter(){ const payload=buildDraftPayloadFromForm(); const models=Array.isArray(payload.Models) ? payload.Models : []; const defaultModelId=payload.Router?.default || models[0]?.id || ''; const candidates=models.slice(0,3).map((model,index)=>({ model:model.id || defaultModelId, description:index===0 ? 'balanced default' : (index===1 ? 'fast fallback' : 'deep fallback') })).filter(item=>item.model); if(!candidates.length && defaultModelId){ candidates.push({ model:defaultModelId, description:'balanced default' }); } payload.SmartRouter={ ...(payload.SmartRouter || {}), enabled:true, analysis_scope:payload.SmartRouter?.analysis_scope || 'last_message', router_model:payload.SmartRouter?.router_model || defaultModelId, fallback:payload.SmartRouter?.fallback || 'default', candidates:(Array.isArray(payload.SmartRouter?.candidates) && payload.SmartRouter.candidates.length) ? payload.SmartRouter.candidates : candidates, rules:Array.isArray(payload.SmartRouter?.rules) ? payload.SmartRouter.rules : [], semantic:{ ...((payload.SmartRouter && payload.SmartRouter.semantic) || {}), enabled:Boolean(payload.SmartRouter?.semantic?.enabled), mode:payload.SmartRouter?.semantic?.mode || 'classifier', classifier_model:payload.SmartRouter?.semantic?.classifier_model || '' }, sticky:{ ...((payload.SmartRouter && payload.SmartRouter.sticky) || {}), enabled:Boolean(payload.SmartRouter?.sticky?.enabled), alignment:{ ...(((payload.SmartRouter && payload.SmartRouter.sticky && payload.SmartRouter.sticky.alignment) || {})), enabled:Boolean(payload.SmartRouter?.sticky?.alignment?.enabled), summarizer_model:payload.SmartRouter?.sticky?.alignment?.summarizer_model || defaultModelId } } }; commitDraftPayload(payload,'已初始化 SmartRouter，可预览后保存'); }` +
    `function addHomeRouteRule(){ const payload=buildDraftPayloadFromForm(); const defaultModelId=payload.Router?.default || payload.Models?.[0]?.id || ''; const smart={ ...((payload && payload.SmartRouter) || {}), enabled:true, analysis_scope:payload.SmartRouter?.analysis_scope || 'last_message', fallback:payload.SmartRouter?.fallback || 'default' }; const rules=Array.isArray(smart.rules) ? smart.rules : []; rules.push({ name:'new-route-'+(rules.length+1), enabled:true, priority:10, model:defaultModelId, description:'从首页新增的路由规则', patterns:[{ type:'exact', keywords:['关键词'] }] }); smart.rules=rules; payload.SmartRouter=smart; commitDraftPayload(payload,'已新增 SmartRouter 规则'); if(advancedConfigDetails){ advancedConfigDetails.open=true; } }` +
    `function addHomeSmartCandidate(){ const payload=buildDraftPayloadFromForm(); const models=Array.isArray(payload.Models) ? payload.Models : []; const defaultModelId=payload.Router?.default || models[0]?.id || ''; const smart={ ...((payload && payload.SmartRouter) || {}), enabled:true, analysis_scope:payload.SmartRouter?.analysis_scope || 'last_message', fallback:payload.SmartRouter?.fallback || 'default' }; const candidates=Array.isArray(smart.candidates) ? smart.candidates : []; candidates.push({ model:defaultModelId, description:'homepage candidate' }); smart.candidates=candidates; payload.SmartRouter=smart; commitDraftPayload(payload,'已新增 SmartRouter 候选'); }` +
    `function clearSmartRouter(){ const payload=buildDraftPayloadFromForm(); delete payload.SmartRouter; delete payload.TriggerRouter; commitDraftPayload(payload,'已清空 SmartRouter 草稿'); }` +
    `function openRouteDesigner(){ if(advancedConfigDetails){ advancedConfigDetails.open=true; advancedConfigDetails.scrollIntoView({ behavior:'smooth', block:'start' }); } }` +
    ...
    `homeRouteRuleList.addEventListener('click',(e)=>{ const btn=e.target.closest('button[data-remove-home-rule]'); if(!btn){ return; } const payload=buildDraftPayloadFromForm(); const smart={ ...((payload && payload.SmartRouter) || {}) }; smart.rules=(Array.isArray(smart.rules) ? smart.rules : []).filter((_,index)=>index!==Number(btn.dataset.removeHomeRule)); if(!smart.rules.length && !smart.candidates?.length && !smart.router_model){ delete payload.SmartRouter; } else { payload.SmartRouter={ ...smart, enabled:true }; } commitDraftPayload(payload,'已删除 SmartRouter 规则'); });` +
    `homeSmartCandidateList.addEventListener('click',(e)=>{ const btn=e.target.closest('button[data-remove-home-candidate]'); if(!btn){ return; } const payload=buildDraftPayloadFromForm(); const smart={ ...((payload && payload.SmartRouter) || {}) }; smart.candidates=(Array.isArray(smart.candidates) ? smart.candidates : []).filter((_,index)=>index!==Number(btn.dataset.removeHomeCandidate)); if(!smart.candidates.length && !smart.rules?.length && !smart.router_model){ delete payload.SmartRouter; } else { payload.SmartRouter={ ...smart, enabled:true }; } commitDraftPayload(payload,'已删除 SmartRouter 候选'); });` +
    ...
    `document.getElementById('initializeSmartRouterBtn').addEventListener('click',initializeSmartRouter);` +
    `document.getElementById('addHomeRouteRuleBtn').addEventListener('click',addHomeRouteRule);` +
    `document.getElementById('addHomeSmartCandidateBtn').addEventListener('click',addHomeSmartCandidate);` +
    `document.getElementById('clearSmartRouterBtn').addEventListener('click',clearSmartRouter);` +
    `document.getElementById('openRouteDesignerBtn').addEventListener('click',openRouteDesigner);` +
