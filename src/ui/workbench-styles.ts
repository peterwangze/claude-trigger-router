export function renderWorkbenchStyles(): string {
  return (
    `:root{color-scheme:light;--bg:#f5f5f7;--panel:#ffffff;--panel-soft:#fbfbfd;--line:#d2d2d7;--line-soft:#e8e8ed;--text:#1d1d1f;--muted:#6e6e73;--brand:#0071e3;--brand-strong:#005bb5;--accent:#34c759;--warn:#b45f06;--critical:#b3261e;--ok:#24a148;--shadow:0 18px 42px rgba(0,0,0,.06);--soft-shadow:0 1px 2px rgba(0,0,0,.04);--radius:8px}` +
    ...
    `.app-shell{display:grid;gap:.75rem;max-width:100%;overflow-x:hidden;background:rgba(255,255,255,.78);border:1px solid rgba(255,255,255,.88);border-radius:var(--radius);box-shadow:var(--shadow);padding:.85rem}` +
    ...
    `.home-flow{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.6rem;margin-bottom:.75rem}` +
    `.home-flow-step{display:grid;grid-template-columns:auto minmax(0,1fr);gap:.08rem .55rem;align-items:center;background:rgba(255,255,255,.82);border:1px solid var(--line-soft);border-radius:var(--radius);padding:.72rem .78rem;box-shadow:var(--soft-shadow);min-width:0}` +
    `.home-flow-step span{grid-row:1/3;display:grid;place-items:center;width:1.55rem;height:1.55rem;border-radius:50%;background:#f5f5f7;color:var(--muted);font-weight:760;font-size:.82rem}` +
    `.home-flow-step strong{font-size:.9rem;line-height:1.2;word-break:break-word}.home-flow-step small{color:var(--muted);font-size:.76rem;line-height:1.2}.home-flow-step.active span{background:var(--brand);color:#fff}` +
    ...
    `.home-router-cards{display:grid;grid-template-columns:1.05fr 1fr 1fr;gap:.75rem;margin-top:.85rem}` +
    `.home-router-card{display:grid;gap:.72rem;align-content:start;background:var(--panel-soft);border:1px solid var(--line-soft);border-radius:var(--radius);padding:.85rem;min-width:0;box-shadow:var(--soft-shadow)}` +
    `.home-card-head{display:flex;justify-content:space-between;gap:.75rem;align-items:flex-start}.home-card-head strong{display:block;font-size:.95rem}.home-card-head .muted{display:block;margin-top:.12rem;font-size:.82rem}` +
    `.home-router-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.5rem}.compact-btn{padding:.38rem .58rem;font-size:.82rem;white-space:nowrap}` +
    `.home-route-list{display:grid;gap:.45rem;min-width:0}.home-route-item{display:flex;justify-content:space-between;gap:.65rem;align-items:center;background:#fff;border:1px solid var(--line-soft);border-radius:var(--radius);padding:.54rem .58rem;min-width:0}.home-route-item span{min-width:0}.home-route-item strong{display:block;font-size:.86rem;line-height:1.2;word-break:break-word}.home-route-item small{display:block;color:var(--muted);font-size:.75rem;line-height:1.25;word-break:break-word}.home-route-empty{background:#fff;border:1px dashed var(--line);border-radius:var(--radius);padding:.62rem .68rem;color:var(--muted);font-size:.84rem}` +
    ...
    `.route-visual-panel{background:linear-gradient(180deg,#fff,#fbfbfd)}` +
    `.route-path-diagram{display:flex;align-items:stretch;gap:.42rem;margin-top:.72rem;overflow-x:auto;padding:.1rem .05rem .35rem}` +
    `.route-stage{min-width:128px;max-width:190px;border:1px solid var(--line-soft);border-radius:var(--radius);background:#fff;padding:.66rem .7rem;display:grid;gap:.18rem;align-content:start;box-shadow:var(--soft-shadow);position:relative}.route-stage span{color:var(--muted);font-size:.72rem;font-weight:700;text-transform:uppercase}.route-stage strong{font-size:.86rem;line-height:1.25;word-break:break-word}.route-stage small{color:var(--muted);font-size:.75rem;line-height:1.28;word-break:break-word}` +
    `.route-stage-input{border-color:#d2d2d7}.route-stage-smart,.route-stage-rule{border-color:#b8dcff;background:#f5faff}.route-stage-pending{border-color:#b8dcff;background:#f5faff}.route-stage-model{border-color:#c8ebd0;background:#fbfffc}.route-edge{display:grid;place-items:center;min-width:28px}.route-edge span{display:block;width:28px;height:1px;background:var(--line);position:relative}.route-edge span:after{content:"";position:absolute;right:-1px;top:-3px;width:7px;height:7px;border-right:1px solid var(--line);border-top:1px solid var(--line);transform:rotate(45deg)}` +
    ...
    `@media (max-width:1080px){.topbar{grid-template-columns:1fr}.topbar-status{justify-self:start}.workspace-grid{grid-template-columns:1fr}.advanced-rail{order:2}.provider-card-grid{grid-template-columns:repeat(auto-fit,minmax(150px,1fr))}.status-strip{grid-template-columns:repeat(2,minmax(0,1fr)) auto}.status-tile:nth-of-type(2n){border-right:0}.home-router-cards{grid-template-columns:1fr 1fr}.home-router-card:first-child{grid-column:1/-1}}` +
    `@media (max-width:760px){body{padding:.35rem}.app-shell{padding:.65rem}.topbar{position:static;align-items:stretch}.surface-tabs{gap:.35rem}.surface-tab{height:auto;border:1px solid var(--line-soft);border-radius:var(--radius);padding:.5rem .7rem}.surface-tab.active{background:#fff;border-color:var(--line-soft);border-bottom-color:var(--line-soft)}.hero h1{font-size:1rem}.status-strip,.role-grid,.ux-checklist,.decision-rail,.config-route-grid,.home-flow,.home-router-cards{grid-template-columns:1fr}.route-path-diagram{display:grid;grid-template-columns:1fr}.route-edge{min-height:18px}.route-edge span{width:1px;height:18px}.route-edge span:after{right:-3px;top:auto;bottom:-1px;transform:rotate(135deg)}.status-tile,.status-tile:nth-of-type(2n){border-right:0;border-bottom:1px solid var(--line-soft);padding:.65rem 0}.status-tile:last-of-type{border-bottom:0}.quick-control-grid,.quick-control-grid>div{grid-template-columns:1fr}.hero-actions,.action-row,.row{align-items:stretch}.hero-actions button,.action-row button,.surface-tab{width:100%}.home-card-head,.home-route-item{align-items:stretch;flex-direction:column}.home-router-actions{grid-template-columns:1fr}.management-table,.trend-table,table{display:block;overflow-x:auto;white-space:nowrap}.panel{padding:.9rem}.provider-card-grid{grid-template-columns:1fr}.advanced-section>summary{max-width:100%}}`
  );
}
