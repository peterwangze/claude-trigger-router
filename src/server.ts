...
export const createServer = (config: any): Server => {
  ...
  server.app.get("/ui", async (_: any, reply: any) => {
    ...
    return reply.send(
      ...
      `        '<div style=\"grid-column:1/-1\"><label>Keywords</label><input data-trigger-pattern-field=\"keywords\" data-index=\"'+index+'\" data-pattern-index=\"'+patternIndex+'\" value=\"'+esc((pattern.keywords || []).join(', '))+'\" placeholder=\"keyword1, keyword2\"'+(pattern.type === 'regex' ? ' disabled' : '')+'><div class=\"muted\">'+(pattern.type === 'regex' ? 'regex 模式下忽略 keywords' : 'exact 模式下按关键词列表匹配')+'</div></div>' +` +
